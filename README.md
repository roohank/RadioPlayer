...
* **Add New Radios:** Easily add new radio stations by providing their name and stream URL.
...
* **Persistent Data:** Radio station lists are saved using `semidbm` and `pickle`.
...
2.  **Install Python dependencies:**
    ```bash
    pip install PyQt6 python-vlc semidbm
    ```
...
5.  **Add Radio:** Click the "Add Radio" button to open a dialog where you can add a new radio station name and its stream link.
...
* **افزودن رادیوهای جدید:** به راحتی ایستگاه‌های رادیویی جدید را با ارائه نام و URL پخش آنها اضافه کنید.
...
* **داده‌های پایدار:** لیست ایستگاه‌های رادیویی با استفاده از `semidbm` و `pickle` ذخیره می‌شوند.
...
5.  **افزودن رادیو:** روی دکمه "Add Radio" کلیک کنید تا یک پنجره باز شود که در آن می‌توانید یک نام ایستگاه رادیویی جدید و لینک پخش آن را اضافه کنید.
